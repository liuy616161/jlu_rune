...
#include "PowerRune.h"
#include rpower_une_node.hpp"

namespace power_rune{


    power_rune::PowerRuneNode(const rclcpp::NodeOptions & options)
    ...
    void power_rune::imageCallback(const sensor_msgs::msg::Image::ConstSharedPrt img_msg)
    {   
        auto img = cv_bridge::toCvShare(img_msg, "rgb8")->image;

        auto start{std::chrono::steady_clock::now()};

        power_rune_->runOnce(image, 0.0, 0.0);
